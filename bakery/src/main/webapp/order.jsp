		...
		<div class="order-form">
			<h1>Thông tin giao hàng</h1>
				...
				<label for="fullname">Họ và tên</label>
				 <input type="text" id="fullname" placeholder="Nhập họ và tên" name="name"> 
				 <label for="phone">Số điện thoại</label> 
				 <input type="text" id="phone"placeholder="Nhập số điện thoại" name="phone"> 
				 <label for="address">Địa chỉ</label> <input type="text" id="address" placeholder="Nhập địa chỉ" name="address">
				...
				<div class="payment-method">
					...
					<div class="payment-option">
						<label> <input type="radio" name="payment" value="cod"
							checked> Thanh toán khi giao hàng (COD)
						</label>
					</div>

					<div class="payment-option">
						<label> <input type="radio" name="payment"
							value="bank-transfer"> Chuyển khoản qua ngân hàng
						</label>
						<div class="bank-details" id="bank-details" style="display: none;">
							<p>Sau khi đặt hàng thành công, Snow Pastry sẽ liên hệ bạn để
								xác nhận đơn hàng.</p>
							<p>Sau đó bạn vui lòng tiến hành chuyển khoản theo số tài
								khoản:</p>
							<p>
								<strong>STK:</strong> 0921004598
							</p>
							<p>
								<strong>NGUYEN MINH PHUONG</strong>
							</p>
							<p>
								<strong>Ngân hàng Quân đội (MB Bank)</strong>
							</p>
						</div>
					</div>
				</div>
	...
	<div class="order-summary">
		...
		<div class="product-item">
			<img src="product/<%=cartItem.getThumbnail()%>"
				alt="<%=cartItem.getProductName()%>"> <span><%=cartItem.getProductName()%></span>
			<span>Số lượng: <%=cartItem.getQuantity()%></span> <span>Thành
				tiền: <%=currencyFormat.format(itemPrice)%></span>
		</div>
		<%
		}
		} else {
		%>
		<p>
			Giỏ hàng của bạn hiện đang trống. <a href="product-list.jsp">Quay
				lại mua sắm</a>.
		</p>
		<%
		}
		...
		<div class="total">
			Tổng cộng:
			<%=currencyFormat.format(totalPrice)%></div>
